use super::{Config, defaults::default_config_toml};
...
pub fn load_config_from_path(custom_path: Option<String>) -> Result<Config> {
    ...
    if config_path.exists() {
        ...
        if is_custom {
            return Err(anyhow::anyhow!("Custom config file not found: {}", config_path.display()));
        }
...
pub fn _get_ascii_art_dir() -> Result<PathBuf> {
    ...
    fs::create_dir_all(&config_dir)
        .with_context(|| format!("Failed to create ASCII art directory: {}", config_dir.display()))?;
